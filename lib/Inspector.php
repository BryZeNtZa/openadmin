 ...
 * File Description : Inspect metadatas
 * Date : Mai 2018.
...
use OpenAdmin\Library\StringLib;
...
class Inspector {
	...
	protected function _parse($comment) {
		...
		$matches = StrLib::match($comment, $pattern);
		
		if ($matches != null) {
			
			foreach ($matches as $match) {
				$parts = ArrayLib::clean( ArrayLib::trim( StrLib::split($match, '[\s]', 2)) );
				$meta[$parts[0]] = true;
	
				if (sizeof($parts) > 1) {
					$meta[$parts[0]] = ArrayLib::clean( ArrayLib::trim( StrLib::split($parts[1], ',')) );
				}
			}
		}
