  ...
  if (file_exists("commun/mpdf/mpdf.php"))
  {
	...
	// Récupération du fichier de style pour l'impression
	  ...
	  $filename2 = "document.pdf";
		...
		if (isset($marge_h) && isset($marge_v))
		{
			if ($orientation=="P")
			{
				$html2pdf = new mPDF('utf-8','A4','','',$marge_h,$marge_h,$marge_v,$marge_v,0,0);
			}
			else
			{
				$html2pdf = new mPDF('utf-8','A4-L','','',$marge_h,$marge_h,$marge_v,$marge_v,0,0);
			}
		}
		else
		{
		  if (isset($taille_page))
			{
				if ($orientation=="P")
				{
					$html2pdf = new mPDF('utf-8',$taille_page,'','',10,10,10,15,0,8);
				}
				else
				{
					$html2pdf = new mPDF('utf-8',$taille_page.'-L','','',10,10,10,15,0,8);
				}
			}
			else
			{
  ...
  else
  {
	...
	// Récupération du fichier de style pour l'impression
	  ...
	  $filename2 = "document.pdf";
		...
		if (isset($marge_h) && isset($marge_v))
		{
		  $contenu=$contenu.'<page backbottom="0">';
		}
		else
		{
			if ($orientation=="P")
